...
static uint64_t reflect_register(const char *str, core_t *cr) {
  ...
  uint64_t reg_addr[72] = {
    ...
    (uint64_t)&(reg->rdi), (uint64_t)&(reg->esi), (uint64_t)&(reg->di), (uint64_t)&(reg->dih), (uint64_t)&(reg->dil),
  ...
  for (int i = 0; i < sizeof(reg_name_list) / sizeof(const char *); i++) {
    if (!strcmp(str, reg_name_list[i])) {
      return reg_addr[i];
    }
  }
    ...
    for (int i = 0; i < str_len; i++) {
      ...
      if (state == 0 && c != ' ') {
        ...
        state = 3;
                    ...
                    (count_parenthese == 0 || count_parenthese == 2))
      {
        state = 4;
        continue; 
      } else if (state == 4 && c != ',' && c != ' ') {
...
static void jne_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
  u_int64_t src = decode_operand(src_od);
